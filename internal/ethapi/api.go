...
// SendPrivateRawTransaction submits a signed transaction to the private pool.
// Private transactions are validated and included in locally-mined blocks but
// are NOT broadcast to the P2P network. This prevents frontrunning/MEV attacks.
func (s *TransactionAPI) SendPrivateRawTransaction(ctx context.Context, input hexutil.Bytes) (common.Hash, error) {
	...
	if err := s.b.SendPrivateTx(ctx, tx); err != nil {
		return common.Hash{}, err
	}
	signer := types.MakeSigner(s.b.ChainConfig(), s.b.CurrentBlock().Number)
	from, _ := types.Sender(signer, tx)
	log.Info("Submitted private transaction", "hash", tx.Hash().Hex(), "from", from)
	return tx.Hash(), nil
}
