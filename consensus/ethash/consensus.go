...
func (ethash *Ethash) VerifyUncles(chain consensus.ChainReader, block *types.Block) error {
	...
	// Verify uncle count limit. After ReducedUncleRewardBlock activation,
	// only 1 uncle is allowed (down from 2) to reduce uncle spam incentive.
...
func (ethash *Ethash) verifyHeader(chain consensus.ChainHeaderReader, header, parent *types.Header, uncle bool, seal bool, unixNow int64) error {
	...
	// After GasLimitBoundsBlock activation, enforce floor and ceiling on gas limit.
...
func CalcDifficulty(config *params.ChainConfig, time uint64, parent *types.Header) *big.Int {
	// AgentChain uses its own difficulty algorithm (no bomb, 6s target)
...
// calcAgentChainBlockReward returns the block reward for AgentChain at the given block number.
// Each epoch (5,256,000 blocks ~= 1 year), reward increases by 2%.
// reward = initialReward * (102/100)^epoch
...
func accumulateRewards(config *params.ChainConfig, state *state.StateDB, header *types.Header, uncles []*types.Header) {
	...
	for _, uncle := range uncles {
		if reducedUncles {
			// After ReducedUncleRewardBlock activation: uncle coinbase gets 1/32
			// of block reward (down from 7/8). This discourages uncle spam and
			// mitigates the Riskless Uncle Making (RUM) attack.
			r.Div(blockReward, big32)
			state.AddBalance(uncle.Coinbase, r)
		} else {
			// Original formula: (uncle_number + 8 - block_number) / 8 * blockReward
		...
		// Miner inclusion reward: 1/32 of block reward per uncle (unchanged)
		r.Div(blockReward, big32)
		reward.Add(reward, r)
	}
