...
// AgentChain difficulty parameters (separate from global constants to avoid
// breaking upstream test suites that depend on mainnet defaults).
const (
	agentChainBlockTarget     = 6   // 6-second block time target
	agentChainMinDifficulty   = 256 // Low minimum for CPU mining
)
...
func CalcDifficultyAgentChain(time uint64, parent *types.Header) *big.Int {
	// (block_timestamp - parent_timestamp) / 6
	...
	// No difficulty bomb for AgentChain
	return y.ToBig()
}
