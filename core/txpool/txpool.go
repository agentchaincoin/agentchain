...
var (
	...
	// ErrUnprotectedTransaction is returned if a transaction is not EIP-155 protected.
	// AgentChain rejects unprotected transactions to prevent cross-chain replay attacks.
	ErrUnprotectedTransaction = errors.New("unprotected transaction rejected (must use EIP-155 with chain ID)")
)
...
var DefaultConfig = Config{
	...
	PriceLimit: 1000000000, // 1 Gwei minimum gas price (prevents free state bloat)
	PriceBump:  100,        // 100% price bump required to replace tx (prevents MEV replacement attacks)
...
type TxPool struct {
	...
	privateTxs map[common.Hash]*types.Transaction // Private transactions (not gossiped)
...
func NewTxPool(config Config, chainconfig *params.ChainConfig, chain blockChain) *TxPool {
	...
	pool.privateTxs = make(map[common.Hash]*types.Transaction)
...
func (pool *TxPool) validateTxBasics(tx *types.Transaction, local bool) error {
	// Reject unprotected (non-EIP-155) transactions to prevent cross-chain replay.
	// Only enforced on AgentChain (chain ID 7331).
	if pool.chainconfig.ChainID != nil && pool.chainconfig.ChainID.Cmp(params.AgentChainChainID) == 0 && !tx.Protected() {
...
// AddPrivateTransaction adds a signed transaction to the private pool.
// Private transactions are validated and added to the regular pool as local
// transactions but are NOT broadcast to the P2P network. They will only be
// included in blocks mined by this node.
func (pool *TxPool) AddPrivateTransaction(tx *types.Transaction) error {
	...
	pool.mu.Lock()
	pool.privateTxs[hash] = tx
	pool.mu.Unlock()

	// Add to the regular pool as a local tx (for inclusion in blocks)
	// but the gossip delay + private flag means it won't be broadcast
	return pool.AddLocal(tx)
}

// GetPrivateTransactions returns all pending private transactions.
func (pool *TxPool) GetPrivateTransactions() []*types.Transaction {
	pool.mu.RLock()
	defer pool.mu.RUnlock()

	txs := make([]*types.Transaction, 0, len(pool.privateTxs))
	for _, tx := range pool.privateTxs {
		txs = append(txs, tx)
	}
	return txs
}

// IsPrivate returns whether a transaction hash is in the private pool.
...
// RemovePrivate removes a transaction from the private pool (e.g., after inclusion).
func (pool *TxPool) RemovePrivate(hash common.Hash) {
	pool.mu.Lock()
	defer pool.mu.Unlock()
