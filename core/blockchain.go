...
func (bc *BlockChain) reorg(oldHead *types.Header, newHead *types.Block) error {
	...
	// Reject excessively deep reorgs to protect against long-range attacks.
	// Always enforced as client-side policy; after MaxReorgDepthBlock activation
	// this becomes a consensus-enforced rule that all nodes must follow.
	const maxReorgDepth = 1000 // ~100 minutes at 6-second blocks
	...
	if depth > maxReorgDepth {
		consensusEnforced := bc.chainConfig.IsMaxReorgDepth(oldHead.Number)
		log.Error("Rejecting deep reorg", "depth", depth, "max", maxReorgDepth, "old", oldNum, "new", newNum,
			"consensus", consensusEnforced)
