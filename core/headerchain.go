...
// cryptoRandIntn returns a crypto-random integer in [0, n).
// Uses crypto/rand to prevent PRNG state prediction attacks during sync.
...
func (hc *HeaderChain) ValidateHeaderChain(chain []*types.Header, checkFreq int) (int, error) {
	...
	if checkFreq != 0 {
		...
		for i := 0; i <= len(seals)/checkFreq; i++ {
			index := i*checkFreq + cryptoRandIntn(checkFreq)
			if index >= len(seals) {
				index = len(seals) - 1
			}
			seals[index] = true
