...
// LoadRandomXConfig loads the stored RandomX config if the chain config
// is already present in database, otherwise, return the config in the
// provided genesis specification. Note the returned RandomX config can
// be nil if we are not in a RandomX network.
func LoadRandomXConfig(db ethdb.Database, genesis *Genesis) (*params.RandomXConfig, error) {
	stored := rawdb.ReadCanonicalHash(db, 0)
	if stored != (common.Hash{}) {
		storedcfg := rawdb.ReadChainConfig(db, stored)
		if storedcfg != nil {
			return storedcfg.RandomX, nil
		}
	}
	if genesis != nil {
		if genesis.Config == nil {
		...
		return genesis.Config.RandomX, nil
	}
	return nil, nil
}
