...
func testTransactionPriceNonceSort(t *testing.T, baseFee *big.Int) {
	...
	for i, txi := range txs {
		...
		// With FCFS ordering, inter-account ordering is by arrival time
		// (not gas price). Since test transactions are created in a fast loop
		// with near-identical timestamps, we only verify nonce ordering within
		// accounts (checked above) and time ordering between accounts.
		if i+1 < len(txs) {
			next := txs[i+1]
			fromNext, _ := Sender(signer, next)
			if fromi != fromNext && txi.time.After(next.time) {
				t.Errorf("invalid time ordering: tx #%d (A=%x T=%v) > tx #%d (A=%x T=%v)", i, fromi[:4], txi.time, i+1, fromNext[:4], next.time)
			}
		}
	}
}
