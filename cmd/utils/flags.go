...
var (
	...
	// RandomX settings
	RandomXThreadsFlag = &cli.IntFlag{
		Name:     "randomx.threads",
		Usage:    "Number of RandomX mining threads (0 = all CPUs)",
		Value:    0,
		Category: flags.EthashCategory,
	}
	RandomXJITFlag = &cli.BoolFlag{
		Name:     "randomx.jit",
		Usage:    "Enable RandomX JIT compilation (default: true)",
		Value:    true,
		Category: flags.EthashCategory,
	}
	RandomXLargePagesFlag = &cli.BoolFlag{
		Name:     "randomx.largepages",
		Usage:    "Enable large memory pages for RandomX",
		Category: flags.EthashCategory,
	}
...
func setRandomX(ctx *cli.Context, cfg *ethconfig.Config) {
	if ctx.IsSet(RandomXThreadsFlag.Name) {
		cfg.RandomX.Threads = ctx.Int(RandomXThreadsFlag.Name)
	}
	if ctx.IsSet(RandomXJITFlag.Name) {
		cfg.RandomX.UseJIT = ctx.Bool(RandomXJITFlag.Name)
	}
	if ctx.IsSet(RandomXLargePagesFlag.Name) {
		cfg.RandomX.UseLargePages = ctx.Bool(RandomXLargePagesFlag.Name)
	}
}
...
func MakeChain(ctx *cli.Context, stack *node.Node, readonly bool) (*core.BlockChain, ethdb.Database) {
	...
	rxChainCfg, err := core.LoadRandomXConfig(chainDb, gspec)
	if err != nil {
		Fatalf("%v", err)
	}
	randomxConfig := ethconfig.Defaults.RandomX
	ethashConfig := ethconfig.Defaults.Ethash
	if ctx.Bool(FakePoWFlag.Name) {
		randomxConfig.PowMode = randomx.ModeFake
		ethashConfig.PowMode = ethash.ModeFake
	}
	var rxCfg *randomx.Config
	if rxChainCfg != nil {
		rxCfg = &randomxConfig
	}
	engine := ethconfig.CreateConsensusEngine(stack, rxCfg, &ethashConfig, cliqueConfig, nil, false, chainDb)
	if gcmode := ctx.String(GCModeFlag.Name); gcmode != "full" && gcmode != "archive" {
		Fatalf("--%s must be either 'full' or 'archive'", GCModeFlag.Name)
	}
